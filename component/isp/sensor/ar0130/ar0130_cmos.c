...
static HI_S32 cmos_get_ae_default(AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    pstAeSnsDft->u32ISPDgainShift = 8;
    pstAeSnsDft->u32MinISPDgainTarget = 1 << pstAeSnsDft->u32ISPDgainShift;		// 左移
    pstAeSnsDft->u32MaxISPDgainTarget = 32 << pstAeSnsDft->u32ISPDgainShift;  
...
/* the function of sensor set fps  帧率设置*/
static HI_VOID cmos_fps_set(HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    pstAeSnsDft->f32Fps = f32Fps;	// 基准帧率
...
// 获取senseor默认的ISP，这些数据都是原厂sensor提供的，我使用者知道大概即可；
HI_U32 cmos_get_isp_default(ISP_CMOS_DEFAULT_S *pstDef)
{   
    if (HI_NULL == pstDef)
    {
...
HI_U32 cmos_get_isp_black_level(ISP_CMOS_BLACK_LEVEL_S *pstBlackLevel)
{
	...
	// 如果sensor的黑电平不会动态根据增益改变，bUpdate配置为HI_FALSE即可；
...
HI_VOID cmos_set_pixel_detect(HI_BOOL bEnable)
{
    ...
    if (bEnable) /* setup for ISP pixel calibration mode  ISP像素校准模式的设置*/
    {
...
HI_VOID cmos_set_wdr_mode(HI_U8 u8Mode)
{
    ...
    switch(u8Mode)
    {
        ...
        case WDR_MODE_NONE:
            ...
            /* Disable 1.25x analog gain 禁用1.25x模拟增益*/
...
// 函数功能: 用于保证AE计算出来的每帧sensor曝光时间和增益配置同步以避免闪烁现象；
...
#ifdef INIFILE_CONFIG_MODE 	// 这个在Makefile中添加了
 ...
 * callback structure                                                       *
 ****************************************************************************/
// 函数作用: 应用层通过这个函数调用驱动，并且和驱动产生关联
int sensor_register_callback(void)
{
    ISP_DEV IspDev = 0;
    HI_S32 s32Ret;
    ALG_LIB_S stLib;	// 主要考虑的是支持多个算法库，并动态切换的情形；
    ISP_SENSOR_REGISTER_S stIspRegister;
    AE_SENSOR_REGISTER_S  stAeRegister;
    AWB_SENSOR_REGISTER_S stAwbRegister;

	// 填充sensor中的数据，里面都是函数指针的挂接
    cmos_init_sensor_exp_function(&stIspRegister.stSnsExp);
	// sensor向ISP注册回调函数
	// AR0130_ID是AR0130的编号
	// IspDev指的是HI3518E中的ISP(位于VI中)，将sensor与VI中的ISP进行绑定；
	// 因为HI3518E中只有一个VI，一个ISP，所以IspDev值为0；
