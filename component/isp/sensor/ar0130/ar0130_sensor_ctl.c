...
int sensor_write_register(int addr, int data)
{
	...
	// 第一次是buf[0]，放的是寄存器地址(低8位放寄存器地址)；
	// buf[idx++]是先寻址，在++；
    buf[idx++] = addr & 0xFF;		// 位与低8位
    if (sensor_addr_byte == 2)		// AR0130寄存器的地址是16位的，所以这里判断是否等于2
    {
		// 第二个参数是对设备的控制命令；
	...
	// 这里的idx是四个字节，2个字节是addr，2个字节是data；
	// 往g_fd文件中写，一次写idx长度，写到buf中；
...
// 这个函数就是sensor驱动，负责处理sensor硬件部分的
void sensor_init_720p_30fps()
{
//[720p30]
	...
	// ethan added
	sensor_write_register( 0x3040, 0x0000); // 未flip、未mirror
	sensor_write_register( 0x3040, 0x8000); // flip、未mirror
