...
*****************************************************************************/
// 这里面的很多函数调用的函数中都是提供操作方法，以函数指针的形式提供，
// 具体实现的部分是在sensor中完成的数据部分填充；
HI_S32 HI_MPI_ISP_Init(ISP_DEV IspDev)
{
    ...
    ISP_AlgsRegister(IspDev);	// 这个函数填充的就是astAlgs结构体变量中的值
