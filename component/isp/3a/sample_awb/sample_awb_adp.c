...
HI_S32 SAMPLE_AWB_IspRegsInit(HI_S32 s32Handle)
{
	...
	// RGB域统计白点信息时，需要设置的相应参数；
    pstStatAttr->u16MeteringWhiteLevelAwb = 0x3ac;	// 默认值，HI3518EV200不支持
    pstStatAttr->u16MeteringBlackLevelAwb = 0x10;	// 默认值是0x40，HI3518EV200不支持
    pstStatAttr->u16MeteringCrRefMaxAwb = 256*2;	// 默认值0x200，HI3518EV200不支持
    pstStatAttr->u16MeteringCrRefMinAwb = 256/2;	// 默认值0x80，HI3518EV200不支持
    ...
    for (i=0; i<4; i++)
    {
		// au16GainOffset值是与具体的sensor有关的，它的值是具体senseor赋值的，在这里
		// 使用了传递的方法，真正的数值由sensor厂商来提供；
...
HI_S32 SAMPLE_AWB_Init(HI_S32 s32Handle, const ISP_AWB_PARAM_S *pstAwbParam)
{
    ...
    pstAwbCtx = AWB_GET_CTX(s32Handle);
    u8Id = AWB_GET_EXTREG_ID(s32Handle);	// 0x4
     ...
     */
     // ALG_LIB_VREG_SIZE = 0x1000，AWB_LIB_VREG_BASE = 0x34000
...
HI_S32 HI_MPI_AWB_Register(ISP_DEV IspDev, ALG_LIB_S *pstAwbLib)
{
	...
	// 函数SAMPLE_AWB_Init、SAMPLE_AWB_Run、SAMPLE_AWB_Ctrl、SAMPLE_AWB_Exit的传参是在
	// sensor_register_callback函数中进行填充的，因为这个函数的实现是在具体的sensor中完成的，
	// 所以里面的传参是需要具体的sensor来完成的；
	// init中的函数需要通过IQ调试来调节视频的白平衡问题
