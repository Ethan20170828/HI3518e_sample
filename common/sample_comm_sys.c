...
这个函数是获取图片的长、宽；用输出型参数(SIZE_S *pstSize)将图片的长、宽输出出来
******************************************************************************/
...
/******************************************************************************
* function : calculate VB Block size of picture.计算图片缓冲池大小=u32VbSize + u32HeaderSize
******************************************************************************/
...
// SAMPLE_COMM_SYS_GetPicSize函数是获取图片的长、宽，用输出型参数将图片的长、宽输出出来
...
// u32VbSize = 宽 * 长 * 每个像素占几字节 = 768 * 1280 * 1.5
// CEILING_2_POWER(stSize.u32Width, u32AlignWidth)含义就是stSize.u32Width数向u32AlignWidt数对齐，直到stSize.u32Width
// (从1280算起，一直累加)数能整除u32AlignWidth这个数为止。
// 这一步处理，增大了缓冲池的大小，为了给图片增加一些余量；
