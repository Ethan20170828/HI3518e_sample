...
#define SAMPLE_SYS_ALIGN_WIDTH      64		// 对齐，如果像素是奇葩数字，则需要对齐操作
...
typedef enum sample_vi_chn_set_e
{
    ...
    VI_CHN_SET_MIRROR,          /* open MIRROR 镜像*/
    VI_CHN_SET_FLIP,            /* open filp 翻转*/
    VI_CHN_SET_FLIP_MIRROR      /* mirror, flip 即镜像又翻转*/
}SAMPLE_VI_CHN_SET_E;
...
// 这个结构体是sample封装的，负责配置VI部分的参数的；
// 里面的各个变量都是VI部分的配置变量，变量的解析都在数据手册VI部分中，直接搜索即可知道结构体含义
typedef struct sample_vi_config_s
{
    SAMPLE_VI_MODE_E enViMode;		// 用的是哪个sensor
    VIDEO_NORM_E enNorm;           /*DC: VIDEO_ENCODING_MODE_AUTO；video的标准*/    
    ROTATE_E enRotate;				// 旋转枚举
