...
#define DefaultTimestampIncrement 3600 //(90000/25)		// 这个是H.264的标准
uint32_t g_userts=0;
...
RtpSession * rtpInit(char *ipStr, int port)
{
	...
	// 这些函数的实现都是在ORTP库中实现好了，我们不需要在进行处理了，
	// 我们只需要将其移植过来，然后进行使用即可；
	// 除非我们有特殊的要求，要自己实现一个ORTP库，或者要修改ORTP库中
	// 的某些参数，才对库中的这些函数中的结构体变量中的值进行修改；
    ortp_init();
    ortp_scheduler_init();
    ...
    // ip指明了是哪个电脑；端口号指明了这个电脑中的哪个应用程序；
    // 网络传输相当于广播式发送，也就是服务器广播发送，客户端接收，如果匹配到，则这次
    // 发送端和接收端匹配上了；
    // ipStr和port参数最后放到数据包内部，作为数据传输在网络中，这个封包其实就是指定了
    // 将来客户端谁能接收到这一包数据了；
...
{  
	...
	// 一个字节数据是在H.264编码时放进去的，指示的一帧数据帧的有效信息；
    unsigned char NALU = buffer[4];

    //如果数据小于MAX_RTP_PKT_LENGTH字节，直接发送：单一NAL单元模式
    if(valid_len <= MAX_RTP_PKT_LENGTH)
    {
		// &buffer[4]是把buffer中的0/1/2/3字节去掉(00 00 00 01)，H.264视频编码后的格式问题；
        sendBytes = rtp_session_send_with_ts(session, &buffer[4], valid_len, g_userts);
    }
	// 要拆分为很多包，一个包一个包的去发送数据
    else if (valid_len > MAX_RTP_PKT_LENGTH)
    {
        //切分为很多个包发送，每个包前要对头进行处理，如第一个包
        valid_len -= 1;
        int k = 0, l = 0;
        k = valid_len / MAX_RTP_PKT_LENGTH;		// 表示有多少个包要发送
        l = valid_len % MAX_RTP_PKT_LENGTH;		// 表示最后一包要发送多少字节
        int t = 0;
        int pos = 5;
        ...
        while(t < k)//||(t==k&&l>0))
        {
            ...
            {
				// 每包数据前面的两个字节内容，用来给下包数据做标识的，来告诉下包内容
				// 存放的位置；其实这些都是H.264编码中的格式；
				// (NALU & 0x60)表示重要性；28表示RTP帧类型；
				// buffer[3]存放的内容表示这包数据要用RTP方式进行发送；
                buffer[pos - 2] = (NALU & 0x60) | 28;
				// (NALU & 0x1f)表示原来的帧类型
                buffer[pos - 1] = (NALU & 0x1f);
				// 这个if语句表示将我们发送过来的第一包置为无效包；
                if(0 == t)
					// buffer[4] |= 0x80;
                    buffer[pos - 1] |= 0x80;

				// 从buffer[3](里面存放的内容表示需要通RTP方式进行发送数据包)开始发送；
...
HI_VOID* SAMPLE_COMM_VENC_GetVencStreamProc(HI_VOID *p)
{
    ...
    ******************************************/
    ...
    for (i = 0; i < s32ChnTotal; i++)
    {
		...
		// 打印结果为 stream_chn0.h264 stream_chn1.h264 stream_chn2.h264；
		// 同时将文件名命名为stream_chn0.h264 stream_chn1.h264 stream_chn2.h264
    ...
    /******************************************
     ...
     整个线程运行其实是一直在这个while循环中执行的，
     当有人把这个变量pstPara->bThreadStart置为HI_FALSE时，线程结束
    ******************************************/
    while (HI_TRUE == pstPara->bThreadStart)
    {
        ...
        TimeoutVal.tv_sec  = 2;
        TimeoutVal.tv_usec = 0;
		// select函数是外部阻塞式，内部非阻塞式，内部在自动轮询检测三路码流，看哪路满足要求；
...
{
	// 用这个变量来做线程限制的，线程内部有个while循环，来检查这个变量，当我们
	// 要结束这个线程时，就把这个变量置为FALSE即可；
